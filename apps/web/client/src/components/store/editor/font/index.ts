...
import { BrandTabValue, type RawFont } from '@onlook/models';
...
export class FontManager {
    ...
    async scanExistingFonts(): Promise<Font[] | undefined> {
        try {
            ...
            const content = await this.editorEngine.sandbox.readFile(normalizePath(layoutPath));
            if (!content) {
                console.log(`Layout file is empty or doesn't exist: ${layoutPath}`);
                return [];
            }

            const { fonts, layoutContent } = extractExistingFontImport(content);
    ...
    async addFont(font: Font) {
        try {
            ...
            // Scan the fonts and load the new font
            await this.scanFonts();
    ...
    private async traverseClassName(
        ...
        try {
            ...
            console.error(`Error traversing className in ${normalizedFilePath}:`, error);
        }
    }
